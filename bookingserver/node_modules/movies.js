...
var listing = function(params, callback) {
	...
	if("ratingorder" in params) {
		var rorder = params.ratingorder;
		retrieveQuery += " ORDER BY rating "+rorder;
	}
	else if("alphaorder" in params) {
		var aorder = params.alphaorder;
		retrieveQuery += " ORDER BY TITLE "+aorder;
	}
	else {}
...
var userbookingInfo = function(params, callback) {
	var bookingsQuery 	= "",
		response 		= { data : {} };
	...
	globals.pool.getConnection(function(err, connection) {
		connection.query(bookingsQuery, function(err, rows) {
			...
			if(err) {
				response.result = "failure";
				response.error 	= "Error in querying. (userbookingInfo)" + err;
			}
			else {
				response.result 		= "success";
				response.data.bookings 	= rows;
...
var editBooking = function(params, callback) {
	var editQuery 	= "START TRANSACTION;",
		response 	= {};

	var seatnum 		= params.seats[0];
		oldticketnum 	= params.tid,
		newticketnum 	= params.sid + seatnum[0] + seatnum[2] + seatnum[3];

	editQuery += 	'UPDATE tickets SET '+
					'TICKETNUM="'+newticketnum+'", '+
					'SEATNUM="'+seatnum+'" '+
					'WHERE TICKETNUM="'+oldticketnum+'"; ';

	editQuery += 	'UPDATE booking SET '+
					'ticketNUM="'+newticketnum+'" '+
					'WHERE ticketNUM="'+oldticketnum+'";'
	...
	globals.pool.getConnection(function(err, connection) {
		connection.query(editQuery, function(err, rows) {
			...
			if(err) {
				response.result = "failure";
				response.error 	= "Error in querying. (editbooking)";
			}
			else {
				response.result = "success";
				response.data 	= rows;
